...
export class StoryAnalysisComponent implements OnInit {
    ...
    if (selectedValue == "consolidate") {
      ...
      if (this.PropertyList.length > 0) {
        ...
        const property = this.PropertyList.filter(
          (p) => selectedValue.includes(p.value)
        );
  ...
  async loadMultipleData(data: any, refreshFilter: boolean) {
    if (!data.properties) {
      ...
      // .getMultipleEntityParentChildData(data) // Before parent child properties
      .getMultipleEntityParentChildData(data)
      .subscribe(
        (response) => {
            console.log(' this.subscription: ',  response);
  ...
  //     this.attributions = JSON.parse(
  //       this.authService.getTotalTopicAttribution()
  //     );
  //   } else {
  //     this.topics = JSON.parse(
  //       this.authService.getTopicData(this.selectedProperty.label)
  //     );
  //     this.attributions = JSON.parse(
  //       this.authService.getTotalAttributionData(this.selectedProperty.label)
  //     );
  //   }
  ...
  loadData(data, refreshFilter) {
    if (this.selectedProperty.propertyName === "consolidate") {
      ...
      this.attributions = JSON.parse(this.authService.getTotalTopicAttribution() || "[]");
        ...
        const allTopics = JSON.parse(this.authService.getTotalTopicData() || "[]");
        const allAttributions = JSON.parse(this.authService.getTotalTopicAttribution() || "[]");

        let filteredTopics: any[] = [];
        let filteredAttributions: any[] = [];
        ...
        selectedList.forEach((property) => {
          const topicData = allTopics.filter((t: any) => t.uuid === property.uuid);
          const attrData = allAttributions.filter((a: any) => a.uuid === property.uuid);

          filteredTopics.push(...topicData);
          filteredAttributions.push(...attrData);
        });

          this.topics = filteredTopics;
          this.attributions = filteredAttributions;

          console.log("Filtered Topics:", this.topics);
          console.log("Filtered Attributions:", this.attributions);
...
// }
  ...
  onTopicSelectionChange(selectedItems: string[]): void {
    ...
    selectedItems.length > 0
      ...
      ? this.updateAttributeListByTopic(selectedItems)
