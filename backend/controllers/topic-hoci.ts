...
export class TopicComponent implements OnInit {
  ...
  this.selectedProperties = selectedValues; // store multiple selected properties

  if (selectedValues && selectedValues.length > 0) {
    // If you need to update topic list for the first selected property
    const firstPropertyId = selectedValues[0];
    this.selectedPropertyId = firstPropertyId;
    this.selectedProperty = this.propertyList.find(p => p.value === firstPropertyId);
    this.updateTopicList(firstPropertyId);
  } else {
    // If no property is selected, clear topic list
  ...
  onUserTopicSelected(value)
  {
    this.showSpinner('sp5');
    const topic = this.topicList.filter(topic => topic.label === value);
    this.selectedTopic = topic ? topic[0] : {};
    this.isEdit = true;  // <--- FIXED
...
// }
  ...
  updateTopicList(propertyId: string, keepSelectedTopic = false) {
    this.topicList = [];
  
    const selectedProperty = this.propertyList.find(prop => prop.value === propertyId);
    if (selectedProperty) {
      this.selectedProperty = selectedProperty;
    }
  
    if (this.selectedProperty?.label?.topics) {
      this.selectedProperty.label.topics.forEach((topic) =>
        this.topicList.push({
          value: topic.values,
          label: topic.key,
          propertyId: this.selectedProperty.value,
          topicId: topic.uuid
        })
      );
    }
